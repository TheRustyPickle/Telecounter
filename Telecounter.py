...
if os.path.exists('resource/kpi_id.pckl') is False:
...
class main_form(QMainWindow):
    ...
    def __init__(self):
        ...
        self.chart_button_color = {self.ui.chart_button_1: 'blue',
                                   ...
                                   self.ui.chart_button_5: (154, 101, 255),
                                   self.ui.chart_button_6: (72, 228, 143),
                                   self.ui.chart_button_7: (255, 195, 0),
        ...
        self.chart_button_color_rgb = {self.ui.chart_button_1: 'blue',
                                       ...
                                       self.ui.chart_button_5: '#9A65FF',
                                       self.ui.chart_button_6: '#48E48F',
                                       self.ui.chart_button_7: '#FFC300',
                                       self.ui.chart_button_8: '#d68c3f',
                                       self.ui.chart_button_9: '#3BE4CA',
    ...
    def connections(self):  # connect all events
        ...
        self.ui.box_starting_mess.textChanged.connect(
            self.text_changed_starting)
        self.ui.tabWidget.currentChanged.connect(self.tab_changed)
        self.ui.Button_count.clicked.connect(self.data_parser)
        self.ui.button_exit.clicked.connect(self.exiting)
        self.ui.table_widget_1.horizontalHeader().sortIndicatorChanged.connect(
                                                        self.sorting_event_1)
        self.ui.table_widget_2.horizontalHeader().sortIndicatorChanged.connect(
                                                        self.sorting_event_2)
        self.button_calender_1.clicked.connect(self.ui.calender_display_1)
        self.button_calender_3.clicked.connect(self.ui.calender_display_1)
        ...
        self.ui.table_widget_2.setVerticalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        self.ui.table_widget_2.setHorizontalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        self.ui.table_widget_1.setVerticalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        self.ui.table_widget_1.setHorizontalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        self.ui.listWidget.setVerticalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        self.ui.listWidget.setHorizontalScrollMode(
            QtWidgets.QAbstractItemView.ScrollPerPixel)
        ...
        self.ui.chart_button_1.clicked.connect(
         lambda state, x=self.ui.chart_button_1: self.deleting_chart_value(x))
        self.ui.chart_button_2.clicked.connect(
         lambda state, x=self.ui.chart_button_2: self.deleting_chart_value(x))
        self.ui.chart_button_3.clicked.connect(
         lambda state, x=self.ui.chart_button_3: self.deleting_chart_value(x))
        self.ui.chart_button_4.clicked.connect(
         lambda state, x=self.ui.chart_button_4: self.deleting_chart_value(x))
        self.ui.chart_button_5.clicked.connect(
         lambda state, x=self.ui.chart_button_5: self.deleting_chart_value(x))
        self.ui.chart_button_6.clicked.connect(
         lambda state, x=self.ui.chart_button_6: self.deleting_chart_value(x))
        self.ui.chart_button_7.clicked.connect(
         lambda state, x=self.ui.chart_button_7: self.deleting_chart_value(x))
        self.ui.chart_button_8.clicked.connect(
         lambda state, x=self.ui.chart_button_8: self.deleting_chart_value(x))
        self.ui.chart_button_9.clicked.connect(
         lambda state, x=self.ui.chart_button_9: self.deleting_chart_value(x))
        self.ui.chart_button_10.clicked.connect(
         lambda state, x=self.ui.chart_button_10: self.deleting_chart_value(x))
    ...
    def modifier(self):  # modify widgets, button before the window loads
        ...
        self.ui.calender.setMaximumDate(
            QDate(int(self.today[0]), int(self.today[1]), int(self.today[2])))
        self.ui.calender.setSelectedDate(
            QDate(int(self.today[0]), int(self.today[0]), 1))
    ...
    def new_buttons(self):
        ...
        self.ui.box_starting_mess.addAction(
            actions, QLineEdit.TrailingPosition)
    ...
    def remove_date_box(self):
        ...
        self.ui.box_starting_mess.setPlaceholderText(
            'Necessary: Message Link To Start From')
        self.ui.box_ending_mess.setPlaceholderText(
            'Optional: Message Link to End At')
    ...
    def sort_empty_buttons(self):
        ...
        for i in self.chart_button_color:
            if i not in self.added_in_chart:
    ...
    def sorting_event_1(self):
        # triggered when clicking on table column buttons for sorting
    ...
    def reload_charts_date(self):
        ...
        self.log_chart.create_chart(
            data=self.date_counts,
            kpi_data=self.date_counts_kpi,
            user_data=self.user_date_counts,
            kpi_selected=kpi_mess_found,
            all_selected=all_mess_found,
            users_to_check=other_values,
            button_colors=self.chart_button_color,
            chart_used_buttons=self.added_in_chart_reverse,
            user_names=self.user_id_name,
    ...
    def reload_charts_hour(self):
        ...
        self.log_chart.create_chart(self.date_hour_counts,
                                    self.date_hour_counts_kpi,
                                    self.user_date_hour_counts,
                                    hourly=True,
                                    kpi_selected=kpi_mess_found,
    ...
    def cell_copier(self):
        ...
        # copies cells selected in the table widget
        self.all_cell_selected = {}
        self.kpi_cell_selected = {}
        self.largest_text_all = {}
        self.largest_text_kpi = {}
        ...
        # sorting is necessary because selectedItems is created by keeping
        # row/column in mind. Sort happens by row => column order
        ...
        for i in self.all_cell_selected:
            self.all_cell_selected[i] = dict(sorted(
                self.all_cell_selected[i].items()))

        for i in self.kpi_cell_selected:
            self.kpi_cell_selected[i] = dict(sorted(
                self.kpi_cell_selected[i].items()))
    ...
    def keyPressEvent(self, event):
        # copy event when pressing CTRL C
    ...
    def text_changed_starting(self):
        ...
        if self.ui.box_ending_date.isVisible() or text != '':
    ...
    def reload_router(self):
        # for checking whether Reload button at ID manager was clicked
    ...
    def reload_kpi(self):
        # reloads pickle kpi data to the list
        global accounts
    ...
    def edit_box_1(self):
        # Paste/Clear button on Message Box
        if self.ui.box_ending_date.isVisible():
            self.ui.box_starting_mess.clear()
            self.ui.box_ending_date.clear()
            ...
            try:
                if starting_data == '':
                    ...
                    if '/c/' in starting_data:
                        starting_data = starting_data.replace(
                            'https://t.me/c/', '')
                        starting_data = "".join(starting_data.split())
    ...
    def datetime_parser(self):
        ...
        if self.add_time is True:
            self.starting_date += datetime.timedelta(
        ...
        if self.add_time is True:
            self.ending_date += datetime.timedelta(
                minutes=self.time_difference)
        else:
            self.ending_date -= datetime.timedelta(
                minutes=self.time_difference)
        ...
        if self.starting_date == self.ending_date:
            self.ui.statusBar().showMessage(
                'Start and Ending date cannot be the same')
            self.clear_statusbar.start()

        elif self.starting_date > self.ending_date:
            self.ui.statusBar().showMessage(
                'Start date cannot be smaller than ending date')
            self.clear_statusbar.start()
    ...
    def date_messages(self, data):
        ...
        for i in data[4]:
            if i not in self.user_date_counts:
        ...
        for i in data[5]:
            if i not in self.user_date_hour_counts:
    ...
    def counting_label(self):
        ...
        if self.counting_time == 0:
            ...
            if self.counting is True and self.finishing_log is True:
                self.ui.statusBar().showMessage(
                    f'Counting and Finishing Log{self.cu_dots}')

            elif self.counting is True and self.finishing_log is False:
                self.ui.statusBar().showMessage(f'Counting{self.cu_dots}')

            elif self.counting is False and self.finishing_log is True:
    ...
    def starting_reset(self):
        ...
        self.empty_buttons = [
            self.ui.chart_button_1,
            self.ui.chart_button_2,
            self.ui.chart_button_3,
            self.ui.chart_button_4,
            self.ui.chart_button_5,
            self.ui.chart_button_6,
            self.ui.chart_button_7,
            self.ui.chart_button_8,
            self.ui.chart_button_9,
            self.ui.chart_button_10]
    ...
    def set_row_data(self, data):
        ...
        # all_log_row format =
        # {user_id:[full_name, user_name, count, row_number, average_char]}
        ...
        if int(data[4]) in self.all_log_row:
            ...
            self.all_log_row[data[4]] = [
                data[0], data[1], int(data[2]), self.all_latest_row_num]
    ...
    def set_row_data_kpi(self, data):
        ...
        if int(data[4]) in self.kpi_log_row:
            ...
            self.kpi_log_row[data[4]] = [
                data[0], data[1], int(data[2]), self.kpi_latest_row_num]
    ...
    def counted_users(self, users):
        ...
        for user in users:
            if users[user] not in self.added_users:
        ...
        if 'All Count' not in self.added_users:
        ...
        for user in self.added_users:
            if user != '' and user != 'KPI Count' and user != 'All Count':
    ...
    def client_starter(self):
        ...
        if self.multi_sess_selected is True:
            ...
            if self.ui.calender.isVisible() or '-' in self.ui.box_starting_mess.text():
                ...
                self.worker = session_verifier(
                    self.group_name, self.cu_session, self.pri_group)
            self.worker.signal.incomplete_sess.connect(self.incom_sess)
            self.worker.signal.finished.connect(self.finishing)
            self.worker.signal.latest_mess.connect(self.latest_mess_id)
            self.worker.signal.date_mess_ids.connect(self.date_message_id)
            pool.start(self.worker)
            self.ui.statusBar().showMessage(
                f'Verifying Session {self.cu_session}')
            try:
                self.thread_timer.timeout.disconnect()
            except Exception:
                pass
    ...
    def single_client(self):
        ...
        if self.mess_id_latest != 0:
            ...
            self.thread_timer.stop()
            self.ui.statusBar().showMessage(
                f'Incomplete Session {self.cu_session}')
            self.enable_widgets()
            self.clear_statusbar.start()
            return
        ...
        for i in range(threadCount):
            self.worker = Worker(
                self.group_name, self.group_starting,
                self.mess_id_latest,
    ...
    def multi_client(self):
        ...
        if self.mess_id_latest != 0 and len(self.session_list) == len(self.incomplete_sess) + len(self.complete_sess):
            ...
            self.ui.statusBar().showMessage(
                f'Could not work with any available session')
            self.thread_timer.stop()
            self.enable_widgets()
            self.clear_statusbar.start()
            return
        ...
        self.ui.statusBar().showMessage(
            f'Working with {len(available_sess)} sessions')
        thread_num = 0
        self.mess_id_latest += 1
        ...
        async def kpi_counter():
            ...
            if me == 'None' or me is None:
                ...
                self.signal.finished.emit(
                    ['incomplete', 'incomplete', self.thread_num])

            else:
                async with client:
                    # start iterating from the selected message links
                    async for message in client.iter_messages(
                         self.group_name, offset_id=self.group_ending):
                        ...
                        if int(mess_id) < self.group_starting:
                            ...
                            try:
                                # get the date in both daily and hourly
                                # in int form add or take time based on
                                # timezone and count the message
                                # amount for each.
                                ...
                                int_date = int(self.date_today.strftime(
                                    "%Y%m%d"))
                                int_hour_date = int(self.date_today.strftime(
                                    "%Y%m%d%H"))
                                ...
                                try:
                                    # another try in case there is no
                                    # message sender
                            ...
                            try:
                                try:
                                    try:
                                        # adds date data, message amount to
                                        # dict for passing it to the charts
                                        ...
                                        if mess_sender in accounts:
                                            ...
                                            int_hour_date = int(
                                                self.date_today.strftime("%Y%m%d%H"))
                                    ...
                                    try:
                                        # keeps track of message length for
                                        # counting average character
                                        mess_len = len(str(mess_text))
                                        # if it's a sticker len is going to
                                        # be 0, so make it 1
                                        if mess_len == 0:
                                            mess_len = 1
                                        self.signal.mess_char.emit(
                                            [mess_sender, mess_len])
                                    except Exception as e:
                                    ...
                                    # pending controls the bar value. So after
                                    # each message add the message value * how
                                    # many many messages. Message passed
                                    # calculated by the last message id that
                                    # was counted - current id
                    ...
                    # in case the bar value does not fill up where it is
                    # supposed to fill it up manually

                    if self.bar > self.max_bar:
                        pass
                    elif self.bar != self.max_bar and self.multi_sess is False:
                        self.bar = self.max_bar

                    elif self.bar != self.max_bar and self.multi_sess is True:
                        self.bar = self.max_bar
                    ...
                    self.signal.date_counts.emit(
                        [self.date_mess_count, self.kpi_mess_count,
                    ...
                    if self.create_log is True:
                        ...
                        for sender in self.message_data:
                            # goes through previously collected
                            # user data for log
                            if stop_process is True:
                                return
                            try:
                                if sender in verified_entities:
                                    ...
                                    verified_entities[sender] = [id_num,
                                                                 username,
                                                                 first_name,
                                                                 last_name]
...
class session_verifier(QRunnable):
    def __init__(self, group_name, session_name, private_group,
                 date_added=False, start_date='', end_date=''):
        super().__init__()
        self.pending = 0
        self.cu_session = session_name
        self.group_name = group_name
    ...
    def run(self):
        async def verifier():
            try:
                ...
                if me == 'None' or me is None:
                    print('Session incomplete')
                    self.signal.incomplete_sess.emit(
                        f'{self.cu_session} is incomplete')

                else:
                    # verifies session whether it can be used for counting
                    async with client:
                        if self.date_added is True:
                            start_mess = 0
                            end_mess = 0
                            new_start_date = int(self.start_date.strftime(
                                                        '%Y%m%d'))
                            start_mess_veri = False

                            while start_mess_veri is False:
                                async for message in client.iter_messages(
                                            self.group_name,
                                            offset_date=self.start_date):

                                    # due to big difference in dates sometimes
                                    # it fails to get any message id so keep
                                    # looping until the message id is found
                                    # numeric date was added so in case the
                                    # selected message is the first message
                                    # in the chat not necessary to add +1 to it

                                    new_mess_date = int(message.date.strftime(
                                                                    '%Y%m%d'))
                            ...
                            async for message in client.iter_messages(
                                 self.group_name, offset_date=self.end_date):
                        ...
                        if self.private_group is True:
                            group_list = []
                            async for group in client.iter_dialogs():
                                group_list.append(group.id)

                        if self.private_group is False or (self.private_group is True and self.group_name in group_list):
                            async for message in client.iter_messages(self.group_name):
                                self.signal.latest_mess.emit(
                                    [message.id, self.cu_session])
                                break

                        elif self.private_group is True and self.group_name not in group_list:
