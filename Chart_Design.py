...
class create(QWidget):
    ...
    def create_chart(self, data={}, kpi_data={}, user_data={}, hourly=False,
                     kpi_selected=True, all_selected=True, users_to_check=[],
                     button_colors={}, chart_used_buttons={},
        ...
        if self.hourly_selected is False:
            # there might be dates missing between messages. So added the
            # missing dates and put the values to 0
            first_key = list(data.keys())[0]
            last_key = list(data.keys())[-1]

            # convert the date INTs to datetime and go through from start to
            # last dates and add all missing dates

            first_date = datetime.datetime(
                int(str(first_key)[:4]), int(str(first_key)[4:6]),
                int(str(first_key)[6:8]))
            last_date = datetime.datetime(
                int(str(last_key)[:4]), int(str(last_key)[4:6]),
                int(str(last_key)[6:8]))
        ...
        elif self.hourly_selected is True:
            first_key = list(data.keys())[0]
            last_key = list(data.keys())[-1]
            ...
            first_date = datetime.datetime(
                int(str(first_key)[:4]), int(str(first_key)[4:6]),
                int(str(first_key)[6:8]), int(str(first_key)[8:]))
            last_date = datetime.datetime(
                int(str(last_key)[:4]), int(str(last_key)[4:6]),
                int(str(last_key)[6:8]), int(str(last_key)[8:]))
        ...
        if self.all_selected is True:
            self.plot.plot(
                axis_x_all, axis_y_all, name="Total Message",
                pen=pg.mkPen(
                    self.button_colors[self.chart_used_buttons['All Count']]))
        else:
            self.plot.plot(axis_x_all, axis_y_all, pen='k')

        if self.kpi_selected is True:
            self.plot.plot(
                axis_x_kpi, axis_y_kpi, name='KPI Message',
                pen=pg.mkPen(
                    self.button_colors[self.chart_used_buttons['KPI Count']]))
        ...
        if self.users_to_check != []:
            for user in self.users_to_check:
                ...
                self.plot.plot(
                  user_x_value, user_y_value,
                  name=f"{legend_name}", pen=pg.mkPen(
                      self.button_colors[self.chart_used_buttons[f'{user}']]))
    ...
    def draw_cursor(self):
        ...
        self.vLine = pg.InfiniteLine(
            angle=90, movable=False, pen=pg.mkPen(
             'k', width=1), label=None, labelOpts={
                    'position': 0.97, 'color': (200, 0, 0),
                ...
                if self.plot.sceneBoundingRect().contains(pos):
                    ...
                    try:
                        ...
                        if self.last_date == date_time:
                            ...
                            if self.hourly_selected is True:
